...
import { ChevronLeft, Users, UserPlus, Filter, Search, Edit, Trash2, Eye, ArrowLeft, Utensils, ShoppingBag, GraduationCap } from 'lucide-react';
...
const TeamMembersList: React.FC = () => {
  ...
  let type: PartnerType = (searchParams.get('type') as PartnerType) || 'store';
  if (location.pathname === '/company/team-members') {
    type = 'collector_company';
  }
  const team = TEAM_MOCKS[type] || [];
  ...
  // Mensagem de vazio
  const emptyMsg = search || filterRole ? 'Nenhum membro encontrado com os filtros atuais.' : 'Nenhum membro cadastrado ainda.';
  ...
  const stats = type === 'restaurant' ? [
    ...
    { label: 'Cupons Servidos', value: '45' },
    { label: 'Avaliação', value: '4.8' },
  ] : type === 'store' ? [
    { label: 'Funcionários', value: '8' },
    { label: 'Cupons Validados', value: '25' },
    { label: 'Avaliação', value: '4.7' },
  ] : [
            ...
            {filteredTeam.length === 0 && (
              <div className="text-center text-muted-foreground py-8">
                {filterStatus === 'Inativo' ? (
                  <>
                    <div className="mb-4">Nenhum membro inativo no momento.<br />Funcionários inativos ficam ocultos das operações do sistema.<br />Você pode reativá-los a qualquer momento.</div>
                    <Button variant="outline" className="bg-green-100 text-green-700 border-green-200 hover:bg-green-200" disabled>Ativar Funcionário (mock)</Button>
                  </>
                ) : filterStatus === 'Excluído' ? (
                  <>
                    <div className="mb-4">Nenhum membro excluído no momento.<br />Funcionários excluídos não aparecem para os demais usuários.<br />Você pode restaurá-los se necessário.</div>
                    <Button variant="outline" className="bg-red-100 text-red-700 border-red-200 hover:bg-red-200" disabled>Restaurar Funcionário (mock)</Button>
                  </>
                ) : (
                  <>
                    {emptyMsg}
                    <div className="mt-4">
                      <Button variant="outline" onClick={() => navigate(getRegisterRoute())}>
                        <UserPlus className="h-4 w-4 mr-2" /> Novo {type === 'cooperative' ? 'Cooperado' : 'Funcionário'}
                      </Button>
                    </div>
                  </>
                )}
          ...
          {editMember && (
            ...
            >
              ...
              <div className="flex gap-2 justify-end">
                <Button variant="outline" type="button" onClick={() => setEditMember(null)}>Cancelar</Button>
