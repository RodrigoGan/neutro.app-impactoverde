...
const CooperativeCollections: React.FC = () => {
  ...
  const [collections, setCollections] = useState([]); // Agora começa vazio
  ...
  // Buscar coletas reais do banco ao carregar
  React.useEffect(() => {
    async function fetchCollections() {
      ...
      try {
        const { data, error } = await supabase
          .from('collections')
          .select('*')
          .eq('collector_id', entityId)
          .order('date', { ascending: false });
        if (error) throw error;
        // Mapear dados do banco para o formato esperado pela UI
        const mapped = (data || []).map((col: any) => ({
          id: col.id,
          client: col.solicitante_nome || col.solicitante_id || 'Cliente',
          avatar: '', // Pode buscar depois se quiser
          status: col.status === 'pending' ? 'Agendada' : col.status === 'scheduled' ? 'Em Andamento' : col.status === 'collected' ? 'Concluída' : col.status === 'cancelled' ? 'Cancelada' : col.status,
          quantity: col.mixed_quantity ? `${col.mixed_quantity}${col.mixed_unit || ''}` : '',
          material: Array.isArray(col.collected_materials) ? col.collected_materials.map(m => m.type || m.material).join(' e ') : '',
          date: col.date,
          time: col.time,
          address: col.address || '',
          driver: col.driver_name || '',
          cancelReason: col.cancel_reason || '',
          fotos: col.photos || [],
          observacoes: col.observations || '',
          materiais: Array.isArray(col.collected_materials) ? col.collected_materials : [],
          observacaoCooperativa: col.observacao_cooperativa || '',
        }));
        setCollections(mapped);
      } catch (err: any) {
        setError('Erro ao buscar coletas da cooperativa.');
        setCollections([]);
      } finally {
        setLoading(false);
      }
    }
    if (entityId) fetchCollections();
  }, [entityId]);
  ...
  const driverOptions = Array.from(new Set(collections.map(c => c.driver)));
